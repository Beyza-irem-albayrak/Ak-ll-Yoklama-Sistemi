  ...
  Future<void> firestorequery() async {
    ...
    try {
      ...
      if (distance > allowedDistance) {
        setState(() {
          hintText = "Konum dışında olduğunuz için yoklama alınmadı.";
        });
        isProcessing = false;
        return;
      }
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        if (haftaList.isNotEmpty && haftaList[0] is Map) {
          ...
          if (passValue == qrText) {
            // Öğrenci zaten eklenmişse tekrar ekleme
            if (!haftaList.contains(widget.number)) {
              haftaList.add(widget.number);
              await docRef.update({week: haftaList});
            } else {
              // Aynı öğrenci daha önce eklenmişse bir daha işlem yapma
              setState(() {
                hintText = "Zaten kaydedilmiş.";
              });
              isProcessing = false;
              return;
            }
